    ...
    checkGameStatus(request, back, old) {
        ...
        if (request.method == Blockly.Gamepad['STATES']['FINISHED'] && !back) {
            if (pegman.x == marker.x && pegman.y == marker.y) 
                 var win = 'lo haz logrado';
                    
            else
                alert('Upsi, Upsi, Dupsi, Te ha faltado algo, revisa tu C�digo =(')
        }
        ...
        if (request.method == Blockly.Gamepad['STATES']['FINISHED'] && !back) {
            ...
            switch (id) {
                ...
                case 6:
                    window.modal6.showModal();
                break;

                case 7:
                    window.modal7.showModal();
                break;

                case 8:
                    window.modal8.showModal();
                break;

                case 9:
                    window.modal9.showModal();
                break;

                case 10:
                    window.modal10.showModal();
                break;

                case 11:
                    window.modal11.showModal();
                break;

                case 12:
                    window.modal12.showModal();
                break;

                case 13:
                    window.modal13.showModal();
                break;

                case 14:
                    window.modal14.showModal();
                break;

                case 15:
                    window.modal15.showModal();
                break;

                case 16:
                    window.modal16.showModal();
                break;

                case 17:
                    window.modal17.showModal();
                break;

                case 18:
                    window.modal18.showModal();
                break;

                case 19:
                    window.modal19.showModal();
                break;
...
/*         console.group()
        ...
        var re = request
        console.log('bloques', JSON.stringify(re))
        
        //var ty =  back ? 'backward' : 'forward'

        //console.log('tipo de solicitud', ty)
        
        var js = pegman
        
        console.log('avanzada', JSON.stringify(pegman))
